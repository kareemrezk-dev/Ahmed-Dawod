...
export function CartSidebar({ locale, dict }: { locale: Locale; dict: any }) {
  ...
  const handleRemoveItem = (id: string, name: string) => {
    if (window.confirm(isAr ? "هل أنت متأكد من الحذف من السلة؟" : "Are you sure you want to remove this from the cart?")) {
      removeFromCart(id);
    }
  };
              ...
              <div className={styles.itemsList}>
                {items.map((item) => (
                  <div key={item.id} className={styles.cartItem}>
                    ...
                    <button className={styles.removeBtn} onClick={() => handleRemoveItem(item.id, `${item.product.brand} ${item.product.modelNumber}`)}>
