  ...
  const filteredOptions = useMemo(() => {
    if (!showSearch || !query.trim()) {
      if (hideOptionsUntilSearch) {
        return activeValue ? options.filter(o => o.value === activeValue) : [];
      }
      return options;
    }
  ...
  return (
    <div className={styles.section}>
      ...
      <div className={needsScroll || showSearch ? styles.scrollableList : ""}>
        ...
        >
          <span className={`${styles.optionDot} ${!activeValue ? styles.optionDotActive : ""}`} />
          <span className={styles.optionLabel}>{allLabel}</span>
          <span className={`${styles.count} ${!activeValue ? styles.countActive : ""}`}>
        ...
        {filteredOptions.length === 0 && query.trim() !== "" ? (
          ...
          filteredOptions.map((opt) => {
            ...
            return (
              ...
              >
                <span className={`${styles.optionDot} ${isActive ? styles.optionDotActive : ""}`} />
...
export function ProductsFilter({
  ...
  return (
    <div className={styles.filter} role="complementary" aria-label={dict.products.filterTitle}>
      {/* Header */}
      ...
      {brandOptions.length > 0 && (
        <FilterSection
          ...
          searchPlaceholder={locale === "ar" ? "بحث بالماركة..." : locale === "en" ? "Search brand..." : "搜索品牌..."}
        />
      )}
      ...
      {sizeOptions.length > 0 && (
        <FilterSection
          ...
          searchPlaceholder={locale === "ar" ? "بحث بالمقاس..." : locale === "en" ? "Search size..." : "搜索尺寸..."}
